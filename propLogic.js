...
export const getVars = (tokens=[]) => { // Token[]
  const vars = {};
...
export const resolveMissingParentheses = (tokens) => { // Token[]
  ...
  else if (total < 0) { // add [-total] opening parentheses to the front
    for (let i = 0; i < -total; i++) {
      tokens.splice(
        0,0,
        new Token({
...
export const executeRPLN = (tokens=[], variables={}) => { // Token[], Record<string,boolean>
...
export const executeAllPossibleRPLN = (tokens=[]) => {
  ...
  for (let i = 2 ** vars.length - 1; i >= 0; i--) {
    ...
    outputs.push(executeRPLN(tokens, varVals));
  }
